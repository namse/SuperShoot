...
void CModel::LoadXFile(wchar_t* strFileName)
{
	//Allocation class
	CAllocateHierarchy Alloc;
	
	//Load the mesh
	if(FAILED(D3DXLoadMeshHierarchyFromX((LPCWSTR)strFileName,		// File load
	...
	{
		MessageBox(NULL, (LPCWSTR)strFileName, L"Model Load Error", MB_OK);
	}
...
void CModel::Update(double dElapsedTime)
{
	...
	if(m_pAnimController && m_dwCurrentAnimation != -1)
		...
		//m_pAnimController->AdvanceTime(m_pAnimController->GetTime()+dElapsedTime, NULL);
