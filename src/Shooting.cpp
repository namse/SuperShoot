...
void InitApp()
{
	...
	// Initialize cameras
	//g_Camera.SetButtonMasks( MOUSE_RIGHT_BUTTON, 0, 0 );
								...
								void* pUserContext )
{
	...
	// Load the meshes
	V_RETURN( g_Background[0].Create( pd3dDevice, L"misc\\seafloor.x" ) );
	...
	D3DXVECTOR3 vecEye( 0.0f, 100.0f, -0.0f );
	D3DXVECTOR3 vecAt ( 0.0f, 0.0f, +0.001f );
	//vecEye	 = CAMERA_INIT_EYE;
	//vecAt = CAMERA_INIT_LOOKAT;
							   ...
							   const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	g_Camera.SetProjParams( D3DX_PI / 4, fAspectRatio, 3.f, INFINITE );
...
void RenderScene( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, bool bRenderLight )
{
	...
	for( auto monster_it = g_game.monster_list_.begin(); monster_it != g_game.monster_list_.end(); monster_it++ )
	{
		...
		D3DXMATRIX mat = *g_MCamera.GetWorldMatrix();
		D3DXMATRIX mat2 = *cam.GetWorldMatrix();

		D3DXMATRIX monster_mat;
		D3DXMatrixIdentity(&monster_mat);
		monster_mat._41 = eye.x;
		monster_mat._42 = eye.y;
		monster_mat._43 = eye.z;

		// Render the mesh
		V( pd3dDevice->SetVertexDeclaration( g_pMeshDecl ) );
		//mWorldView = g_mWorldScaling * *g_MCamera.GetWorldMatrix() * *g_Camera.GetViewMatrix();
		mWorldView = g_mWorldScaling * monster_mat * *g_Camera.GetViewMatrix();
		mWorldViewProjection = mWorldView * *g_Camera.GetProjMatrix();
		V( g_pEffect->SetMatrix( "g_mWorldViewProjection", &mWorldViewProjection ) );
		V( g_pEffect->SetMatrix( "g_mWorldView", &mWorldView ) );
		V( g_pEffect->Begin( &cPasses, 0 ) );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	{
		pd3dDevice->SetViewport(&g_viewport);   
						...
						bool bSideButton2Down, int nMouseWheelDelta, int xPos, int yPos, void* pUserContext )
{
	...
	*/
	...
	if(bLeftButtonDown == true)
	{
		...
		D3DXMATRIX proj_inverse;
		D3DXMATRIX view_inverse;
		float deter;

		deter = D3DXMatrixDeterminant( g_Camera.GetViewMatrix() );
		D3DXMatrixInverse( &view_inverse, &deter, g_Camera.GetViewMatrix() );

		deter = D3DXMatrixDeterminant( g_Camera.GetWorldMatrix() );
		D3DXMatrixInverse( &world_inverse, &deter, g_Camera.GetWorldMatrix() );

		deter = D3DXMatrixDeterminant( g_Camera.GetProjMatrix() );


		D3DXMatrixInverse( &proj_inverse, &deter, g_Camera.GetProjMatrix() );
		D3DXMATRIX matInverse;
		// use the mouse coordinates to get the mouse angle
		float xAngle = (((2.0f * xPos) / DXUTGetD3D9BackBufferSurfaceDesc()->Width) - 1.0f) / (*(g_Camera.GetProjMatrix()))(0, 0);
		float yAngle = (((-2.0f * yPos) / DXUTGetD3D9BackBufferSurfaceDesc()->Height) + 1.0f) / (*(g_Camera.GetProjMatrix()))(1, 1);
		...
		D3DXVECTOR3 from = *( g_Camera.GetEyePt() );
		from.x += ( from.y / direction.z ) * direction.x;
		from.z += ( from.y / direction.z ) * direction.y;
		from.y = 0;


		printf("%.2f / %.2f/ %.2f -> %.2f / %.2f / %.2f\n",direction.x, direction.y, direction.z, from.x, from.y, from.z);
