...
CBaseCamera::CBaseCamera()
{
    ...
    SetProjParams( D3DX_PI / 4, 1.0f, 3.0f, INFINITE);
...
VOID CBaseCamera::SetViewParams( D3DXVECTOR3* pvEyePt, D3DXVECTOR3* pvLookatPt )
{
    ...
    //D3DXVECTOR3 vUp( 20.f, 20.f*1.41f, 20.f );
...
LRESULT CModelViewerCamera::HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	game_->ScreenClicked(ptCursor.x, ptCursor.y);
	...
	world.m[0][0] = world.m[1][1] = world.m[2][2] = 1.f;
	//D3DXVec3Unproject(&out, &in, &g_viewport, &m_mProj, &m_mView, &world);

	//printf("%.1f / %.1f / %.1f\n", out.x, out.y, out.z);

	out = m_ViewArcBall.ScreenToVector(ptCursor.x, ptCursor.y);
	printf("%.3f / %.3f / %.3f\n", out.x, out.y, out.z);
	}
